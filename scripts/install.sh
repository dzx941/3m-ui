...
arch_name() {
    case "$(uname -m)" in x86_64|amd64) echo amd64 ;; aarch64|arm64) echo arm64 ;; armv7l|armv7*) echo armv7 ;; *) echo "Unsupported architecture" >&2; exit 1 ;; esac
}
os_id() { if [ -r /etc/os-release ]; then . /etc/os-release; echo "${ID:-unknown}"; else echo unknown; fi; }
...
asset_suffix() {
    case "${THREE_M_UI_STATIC:-auto}" in
        1|true|yes) echo "-static" ;; 0|false|no) echo "" ;;
        *) if [ -r /etc/os-release ] && grep -q '^ID=alpine' /etc/os-release; then echo "-static"; elif [ -e /lib/ld-musl-x86_64.so.1 ] || [ -e /lib/ld-musl-aarch64.so.1 ] || [ -e /lib/ld-musl-armhf.so.1 ]; then echo "-static"; else echo ""; fi ;;
    esac
}
mihomo_asset() {
    case "$(uname -m)" in x86_64|amd64) echo "mihomo-linux-amd64-compatible" ;; aarch64|arm64) echo "mihomo-linux-arm64" ;; armv7l|armv7*) echo "mihomo-linux-armv7" ;; *) return 1 ;; esac
}
latest_mihomo_tag() {
    tag=""
    if command_exists curl; then tag="$(curl -fsSLI -o /dev/null -w '%{url_effective}' https://github.com/MetaCubeX/mihomo/releases/latest 2>/dev/null | sed 's#.*/##')" || true; fi
    case "$tag" in v[0-9]*) echo "$tag" ;; *) echo "" ;; esac
}
install_mihomo() {
    if [ -x "$MIHOMO_BIN" ] && "$MIHOMO_BIN" -v >/dev/null 2>&1; then echo "Mihomo already installed and validated."; return 0; fi
    mkdir -p "$DATA_DIR/mihomo"; asset="$(mihomo_asset)"; tag="$(latest_mihomo_tag)"; [ -n "$tag" ] || { echo "Unable to determine the latest Mihomo release." >&2; return 1; }
    tmp="$(mktemp)"; stage="$(mktemp)"; url="https://github.com/MetaCubeX/mihomo/releases/download/${tag}/${asset}-${tag}.gz"; echo "Downloading Mihomo ${tag}..."
    if ! download "$url" "$tmp" || [ ! -s "$tmp" ]; then echo "Failed to download Mihomo: $url" >&2; rm -f "$tmp" "$stage"; return 1; fi
    if ! gzip -dc "$tmp" > "$stage" || [ ! -s "$stage" ]; then echo "Failed to extract Mihomo." >&2; rm -f "$tmp" "$stage"; return 1; fi
...
write_config() {
    ...
    jwt_secret="$(random_hex 32)"; cred_key="$(random_hex 32)"
...
arch="$(arch_name)"; tmp="$(mktemp)"; panel_url="https://github.com/$REPO/releases/latest/download/3m-ui-linux-${arch}$(asset_suffix)"; echo "Downloading 3m-ui..."
...
echo "3m-ui installed successfully"; echo "Mihomo is managed by 3m-ui and is validated before startup."; echo "Open http://SERVER_IP:8080/"
