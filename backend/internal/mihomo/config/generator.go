	...
	for _, l := range listeners {
		...
		case "shadowsocks":
			if len(listenerCreds) > 1 {
				return nil, fmt.Errorf("listener %q: Shadowsocks supports one password; %d active credentials are bound", l.Name, len(listenerCreds))
			}
			copyOption(m, configMap, "cipher")
			if len(listenerCreds) == 1 && listenerCreds[0].Password != "" {
				m["password"] = listenerCreds[0].Password
			} else {
				copyOption(m, configMap, "password")
			}
		...
		case "vmess", "vless":
			...
			for _, cred := range listenerCreds {
				...
				if flow, ok := configMap["flow"]; ok && protocol == "vless" {
					u["flow"] = flow
				}
				if alterID, ok := configMap["alterId"]; ok && protocol == "vmess" {
					u["alterId"] = alterID
				}
...
func normalizeListenerUserList(value interface{}) ([]map[string]interface{}, error) {
	...
	case map[interface{}]interface{}:
		...
		for rawUsername, rawPassword := range users {
			...
			result = append(result, map[string]interface{}{
				"username": username,
				"password": fmt.Sprint(rawPassword),
			})
		}
		return result, nil
	default:
...
func listenerFieldIsManaged(key string) bool {
	...
	case "users", "username", "password", "uuid", "flow", "alterId",
		...
		"restls-opts", "jls-opts", "ws-opts", "grpc-opts", "h2-opts",
		"http-opts", "mkcp-opts", "certificate", "private-key", "private_key":
		return true
	default:
		return false
	}
