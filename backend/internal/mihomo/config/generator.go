...
type Credential struct { Username string; Password string; UUID string }

// CredentialProvider is supplied by the application composition root. The
// config package must not import user: user -> node -> config already exists.
...
func (ce *ConfigEngine) GenerateFinalConfig() (string, error) {
	baseBytes, err := yaml.Marshal(GetDefaultTemplate()); if err != nil { return "", err }
	var merged map[string]interface{}; if err := yaml.Unmarshal(baseBytes, &merged); err != nil { return "", err }
	...
	for _, fragment := range customFragments { var fragMap map[string]interface{}; if err := yaml.Unmarshal([]byte(fragment.Content), &fragMap); err != nil { return "", fmt.Errorf("invalid custom config %q: %w", fragment.Name, err) }; for k, v := range fragMap { merged[k] = v } }
	...
	if CredentialProvider != nil { credentials, err = CredentialProvider(); if err != nil { return "", fmt.Errorf("load listener credentials: %w", err) } }
	generated, err := generateListeners(listeners, credentials); if err != nil { return "", err }
	merged["listeners"] = generated
	finalBytes, err := yaml.Marshal(merged); if err != nil { return "", fmt.Errorf("serialize final configuration: %w", err) }
...
func generateListeners(listeners []models.Listener, creds map[uint][]Credential) ([]map[string]interface{}, error) {
	...
	for _, l := range listeners {
		...
		protocol := strings.ToLower(strings.TrimSpace(l.Protocol)); if protocol == "" { protocol = strings.ToLower(strings.TrimSpace(l.Type)) }
		if !IsMihomoListenerProtocol(protocol) { return nil, fmt.Errorf("unsupported Mihomo listener protocol %q", protocol) }
		if l.Port < 1 || l.Port > 65535 { return nil, fmt.Errorf("listener %q has invalid port %d", l.Name, l.Port) }
		listen := strings.TrimSpace(l.BindAddress); if listen == "" { listen = strings.TrimSpace(l.Listen) }; if listen == "" { listen = "0.0.0.0" }
		m := map[string]interface{}{"name": l.Name, "type": protocol, "port": l.Port, "listen": listen}
		configMap, err := decodeListenerConfig(l.Config); if err != nil { return nil, fmt.Errorf("listener %q: %w", l.Name, err) }
		if l.Proxy != "" { m["proxy"] = l.Proxy }; if l.Rule != "" { m["rule"] = l.Rule }; if value, ok := configMap["routing-mark"]; ok { m["routing-mark"] = value }; if l.UDP && listenerHasUDPOption(protocol) { m["udp"] = true }
		for k, v := range configMap { if listenerFieldIsManaged(k) { continue }; m[k] = v }
		copyServerTLSFields(m, configMap); listenerCreds := creds[l.ID]
		switch protocol {
		case "shadowsocks":
			if len(listenerCreds) > 1 { return nil, fmt.Errorf("listener %q: Shadowsocks supports one password; %d active credentials are bound", l.Name, len(listenerCreds)) }; copyOption(m, configMap, "cipher"); if len(listenerCreds) == 1 && listenerCreds[0].Password != "" { m["password"] = listenerCreds[0].Password } else { copyOption(m, configMap, "password") }
		case "snell": copyOption(m, configMap, "psk"); copyOption(m, configMap, "version")
		case "vmess", "vless":
			users := make([]map[string]interface{}, 0, len(listenerCreds)); for _, cred := range listenerCreds { if cred.UUID == "" { continue }; u := map[string]interface{}{"uuid": cred.UUID}; if cred.Username != "" { u["username"] = cred.Username }; if flow, ok := configMap["flow"]; ok && protocol == "vless" { u["flow"] = flow }; if alterID, ok := configMap["alterId"]; ok && protocol == "vmess" { u["alterId"] = alterID }; users = append(users, u) }; if len(users) > 0 { m["users"] = users } else if value, ok := configMap["users"]; ok { m["users"] = value }
		case "trojan":
			users := make([]map[string]interface{}, 0, len(listenerCreds)); for _, cred := range listenerCreds { if cred.Password == "" { continue }; u := map[string]interface{}{"password": cred.Password}; if cred.Username != "" { u["username"] = cred.Username }; users = append(users, u) }; if len(users) > 0 { m["users"] = users } else if value, ok := configMap["users"]; ok { m["users"] = value }
		case "hysteria2", "anytls", "mieru":
			users := make(map[string]string); for _, cred := range listenerCreds { if cred.Username != "" && cred.Password != "" { users[cred.Username] = cred.Password } }; if len(users) > 0 { m["users"] = users } else if value, ok := configMap["users"]; ok { m["users"] = value }
		case "tuic":
...
func listenerHasUDPOption(protocol string) bool { return protocol == "shadowsocks" || protocol == "snell" }
