...
func listenerFields(values ...string) map[string]struct{} {
	...
	for _, value := range values { fields[value] = struct{}{} }
	return fields
}

// listenerNested accepts dotted paths and creates an exact schema for every
// object level, e.g. reality-config.limit-fallback-upload.after-bytes.
func listenerNested(values ...string) map[string]map[string]struct{} {
	...
	for _, value := range values {
		...
		for i := 0; i < len(parts)-1; i++ {
			...
			child := parts[i+1]
			if fields[parent] == nil { fields[parent] = make(map[string]struct{}) }
			fields[parent][child] = struct{}{}
		}
	}
	return fields
}
...
func splitPath(value string) []string {
	...
	for i := 0; i <= len(value); i++ {
		if i == len(value) || value[i] == '.' {
			if i > start { out = append(out, value[start:i]) }
			start = i + 1
		}
	}
	return out
...
var MihomoListenerSchemas = map[string]ListenerSchema{
	"shadowsocks": {Protocol: "shadowsocks", Fields: listenerFields("cipher", "password", "udp", "shadow-tls", "kcp-tun"), NestedFields: listenerNested(
		"shadow-tls.enable", "shadow-tls.version", "shadow-tls.password", "shadow-tls.users", "shadow-tls.handshake.dest",
