...
func TestApplyConfigCreatesBackupAndRollbackRestoresPreviousConfig(t *testing.T) {
	...
	if err := svc.ApplyConfig(candidate); err != nil {
	...
	if string(got) != candidate {
		t.Fatalf("applied config = %q, want %q", got, candidate)
	}
	...
	if err := svc.RollbackConfig(); err != nil {
	...
	if string(got) != old {
		t.Fatalf("rolled back config = %q, want %q", got, old)
	}
}
