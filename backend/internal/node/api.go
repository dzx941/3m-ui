...
func CreateNode(c *gin.Context) {
	...
	if err := GlobalService.Create(&l); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func UpdateNode(c *gin.Context) {
	...
	l.ID = uint(id)
	if err := ValidateNode(&l); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
	if err := GlobalService.Update(&l); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
	c.JSON(http.StatusOK, l)
}
...
func DeleteNode(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid node id"}); return }
	if err := GlobalService.Delete(uint(id)); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func ReloadNode(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 32)
	if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "invalid node id"}); return }
	if err := GlobalService.TriggerReload(uint(id)); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func CreateClientAccess(c *gin.Context) {
	...
	if err := database.GlobalDB.First(&listener, uint(id)).Error; err != nil { c.JSON(http.StatusNotFound, gin.H{"error": "listener not found"}); return }
	if !listener.Enabled { c.JSON(http.StatusBadRequest, gin.H{"error": "listener is disabled"}); return }
	...
	if err := database.GlobalDB.Where("listener_id = ? AND enabled = ?", listener.ID, true).First(&existing).Error; err == nil {
		if existing.ExpireAt == nil || existing.ExpireAt.After(time.Now()) { c.JSON(http.StatusOK, clientAccessResponse(c, existing)); return }
	}
	buf := make([]byte, 16)
	...
	if err := database.GlobalDB.Create(&token).Error; err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
