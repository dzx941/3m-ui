...
func snellURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	if psk == "" { return nil, fmt.Errorf("snell listener requires psk for URI export") }
	params := map[string]string{"psk": psk}
	if v := stringValue(cfg["version"], "4"); v != "" { params["version"] = v }
	if obfs, ok := cfg["obfs-opts"].(map[string]interface{}); ok {
		if mode := stringValue(obfs["mode"], ""); mode != "" { params["obfs"] = mode }
		if host := stringValue(obfs["host"], ""); host != "" { params["obfs-host"] = host }
	}
	if b, ok := cfg["udp"].(bool); ok && b { params["udp"] = "1" }
...
func shadowQUICURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	users := userMap(cfg)
	if len(users) == 0 { return nil, fmt.Errorf("shadowquic listener requires at least one user for URI export") }
	result := make([]string, 0, len(users))
	for username, raw := range users {
		password, ok := raw.(string)
		if !ok || password == "" { return nil, fmt.Errorf("shadowquic user %q has empty password", username) }
		...
		if b, ok := cfg["zero-rtt"].(bool); ok && b { params["zero-rtt"] = "1" }
		if b, ok := cfg["udp-over-stream"].(bool); ok && b { params["udp-over-stream"] = "1" }
		if b, ok := cfg["disable-mtu-discovery"].(bool); ok && b { params["disable-mtu-discovery"] = "1" }
...
func mieruURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	users := userMap(cfg)
	if len(users) == 0 { return nil, fmt.Errorf("mieru listener requires at least one user for URI export") }
	result := make([]string, 0, len(users))
	for username, raw := range users {
		password, ok := raw.(string)
		if !ok || password == "" { return nil, fmt.Errorf("mieru user %q has empty password", username) }
		params := map[string]string{}
...
func trustTunnelURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	users := userMap(cfg)
	if len(users) == 0 { return nil, fmt.Errorf("trusttunnel listener requires at least one user for URI export") }
	result := make([]string, 0, len(users))
	for username, raw := range users {
		password, ok := raw.(string)
		if !ok || password == "" { return nil, fmt.Errorf("trusttunnel user %q has empty password", username) }
		params := map[string]string{}
		...
		for _, key := range []string{"udp", "quic", "skip-cert-verify", "name-cert-verify"} {
			if b, ok := cfg[key].(bool); ok && b { params[key] = "1" }
		}
