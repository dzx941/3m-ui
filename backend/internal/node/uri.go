...
// ClientURIs converts one Listener into client-compatible URIs. A listener
// with multiple users returns one URI per user. The listener bind address is
// never exported as a public endpoint when it is a wildcard; callers should
// pass the public host resolved from the request/configuration.
...
func normalizeExportHost(requestHost, bind, listen string) string {
	h := strings.TrimSpace(requestHost)
	if h != "" {
		if host, _, err := net.SplitHostPort(h); err == nil { return strings.Trim(host, "[]") }
		return strings.Trim(h, "[]")
	}
	for _, candidate := range []string{bind, listen} {
		candidate = strings.TrimSpace(candidate)
		if candidate == "" || candidate == "0.0.0.0" || candidate == "::" || candidate == "*" || candidate == "127.0.0.1" || candidate == "::1" { continue }
		if host, _, err := net.SplitHostPort(candidate); err == nil { return strings.Trim(host, "[]") }
...
func vlessURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	for _, row := range rows {
		...
		if _, ok := cfg["reality-config"]; ok { params["security"] = "reality"; if rc, ok := cfg["reality-config"].(map[string]interface{}); ok { params["pbk"], _ = rc["public-key"].(string); if params["pbk"] == "" { params["pbk"], _ = rc["private-key"].(string) }; params["sid"], _ = rc["short-id"].(string); params["sni"], _ = firstString(rc["server-names"]) } }
		if ws, ok := cfg["ws-path"].(string); ok && ws != "" { params["type"] = "ws"; params["path"] = ws }
		if grpc, ok := cfg["grpc-service-name"].(string); ok && grpc != "" { params["type"] = "grpc"; params["serviceName"] = grpc }
		result = append(result, query("vless://"+url.PathEscape(uuid)+"@"+net.JoinHostPort(host, port), params)+"#"+url.PathEscape(name))
	}
	return result, nil
}
...
func vmessURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	result := make([]string, 0, len(rows))
	for _, row := range rows {
		obj := map[string]string{"v":"2", "ps":name, "add":host, "port":port, "id":fmt.Sprint(row["uuid"]), "aid":"0", "scy":"auto", "net":"tcp", "type":"none", "tls":""}
		...
		if _, ok := cfg["reality-config"]; ok { obj["tls"]="reality" }
...
func trojanURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	result := make([]string, 0, len(rows))
	for _, row := range rows { password, _ := row["password"].(string); if password == "" { return nil, fmt.Errorf("trojan user password is required") }; params := map[string]string{}; if s, _ := cfg["sni"].(string); s != "" { params["sni"] = s }; result = append(result, query("trojan://"+url.PathEscape(password)+"@"+net.JoinHostPort(host, port), params)+"#"+url.PathEscape(name)) }
	return result, nil
}
...
func hysteria2URIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	result := make([]string, 0, len(users)); for user, raw := range users { password, _ := raw.(string); if password == "" { return nil, fmt.Errorf("hysteria2 user %q has empty password", user) }; params := map[string]string{}; if s, _ := cfg["sni"].(string); s != "" { params["sni"] = s }; if bool, _ := cfg["insecure"].(bool); bool { params["insecure"]="1" }; result = append(result, query("hysteria2://"+url.PathEscape(user)+":"+url.PathEscape(password)+"@"+net.JoinHostPort(host, port), params)+"#"+url.PathEscape(name)) }; return result, nil
}
...
func tuicURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	result := make([]string, 0, len(users)); for user, raw := range users { password, _ := raw.(string); if password == "" { return nil, fmt.Errorf("tuic user %q has empty password", user) }; result = append(result, "tuic://"+url.PathEscape(user)+":"+url.PathEscape(password)+"@"+net.JoinHostPort(host, port)+"#"+url.PathEscape(name)) }; return result, nil
}
...
func anytlsURIs(name, host, port string, cfg map[string]interface{}) ([]string, error) {
	...
	result := make([]string, 0, len(users)); for user, raw := range users { password, _ := raw.(string); if password == "" { return nil, fmt.Errorf("anytls user %q has empty password", user) }; params := map[string]string{}; if s, _ := cfg["sni"].(string); s != "" { params["sni"] = s }; if insecure, _ := cfg["allow-insecure"].(bool); insecure { params["insecure"]="1" }; result = append(result, query("anytls://"+url.PathEscape(user)+":"+url.PathEscape(password)+"@"+net.JoinHostPort(host, port), params)+"#"+url.PathEscape(name)) }; return result, nil
}
