...
func ClientURIsWithCredentials(listener models.Listener, host string, credentials []user.Credential) ([]string, error) {
	...
	if len(credentials) > 0 {
		...
		case "anytls", "hysteria2", "mieru", "tuic":
			...
			for _, credential := range credentials { if credential.Username != "" { users[credential.Username] = credential.Password } }
			cfg["users"] = users
		default:
		...
		if listener.Protocol == "shadowsocks" { cfg["password"] = credentials[0].Password }
	}
	encoded, err := json.Marshal(cfg)
	if err != nil { return nil, fmt.Errorf("failed to prepare URI configuration: %w", err) }
