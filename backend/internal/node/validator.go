...
func validateSchema(proto string, cfg map[string]interface{}) error {
	schema, ok := mihomoConfig.GetMihomoListenerSchema(proto); if !ok { return fmt.Errorf("no listener schema registered for protocol %s", proto) }
...
func validateObject(proto, prefix string, value map[string]interface{}, allowed map[string]struct{}, nested map[string]map[string]struct{}) error {
	for key, child := range value {
		...
		if childMap, ok := child.(map[string]interface{}); ok {
			...
			for nestedKey, nestedAllowed := range nested { if strings.HasPrefix(nestedKey, key+".") { childNested[strings.TrimPrefix(nestedKey, key+".")] = nestedAllowed } }
			if err := validateObject(proto, joinField(prefix, key), childMap, childAllowed, childNested); err != nil { return err }
		}
	}
	return nil
}
...
func validateProtocolSpecific(proto string, cfg map[string]interface{}) error {
	switch proto {
	case "anytls": return validateAnyTLS(cfg)
	case "snell": if value, ok := numeric(cfg["version"]); ok && (value < 1 || value > 5) { return fmt.Errorf("snell version must be between 1 and 5") }
	case "hysteria2":
	...
	case "trojan", "shadowquic":
		users, ok := cfg["users"].([]interface{}); if !ok || len(users) == 0 { return fmt.Errorf("%s listener requires at least one user", proto) }; for i, user := range users { if err := validateUserRow(proto, i, user, false); err != nil { return err } }
	case "mieru": if users, ok := cfg["users"].(map[string]interface{}); !ok || len(users) == 0 { return fmt.Errorf("mieru listener requires at least one user") }
	case "sudoku":
		min, minOK := numeric(cfg["padding-min"]); max, maxOK := numeric(cfg["padding-max"]); if minOK && maxOK && max < min { return fmt.Errorf("sudoku padding-max must be greater than or equal to padding-min") }
	}
	return nil
}
...
func validateAnyTLS(cfg map[string]interface{}) error {
	...
	if cert != key { return fmt.Errorf("anytls listener requires certificate and private-key together") }
	shadow, res, jls := enabledObject(cfg["shadow-tls"]), enabledObject(cfg["res-tls"]), enabledObject(cfg["jls-config"])
	secureModes := 0; if shadow { secureModes++ }; if res { secureModes++ }; if jls { secureModes++ }; if cert { secureModes++ }
	if secureModes > 1 { return fmt.Errorf("anytls listener TLS modes are mutually exclusive: use certificate/private-key, shadow-tls, res-tls, or jls-config") }
	if !boolValue(cfg["allow-insecure"]) && secureModes == 0 { return fmt.Errorf("anytls listener requires certificate/private-key, shadow-tls, res-tls, jls-config, or allow-insecure=true") }
	if auth, ok := cfg["client-auth-type"].(string); ok && (auth == "verify-if-given" || auth == "require-and-verify") && !hasString(cfg["client-auth-cert"]) { return fmt.Errorf("anytls listener client-auth-cert is required for %s", auth) }
	users, ok := cfg["users"].(map[string]interface{}); if !ok || len(users) == 0 { return fmt.Errorf("anytls listener requires at least one user") }
	for username, password := range users { if strings.TrimSpace(username) == "" || !hasString(password) { return fmt.Errorf("anytls listener users must contain non-empty username/password pairs") } }
	return nil
}

func enabledObject(v interface{}) bool { m, ok := v.(map[string]interface{}); return ok && boolValue(m["enable"]) }
func validateUserRow(proto string, index int, raw interface{}, uuidMode bool) error {
	row, ok := raw.(map[string]interface{}); if !ok { return fmt.Errorf("%s listener users[%d] must be an object", proto, index) }
	allowed := map[string]struct{}{"username": {}}; if uuidMode { allowed["uuid"] = struct{}{}; if proto == "vmess" { allowed["alterId"] = struct{}{} } else { allowed["flow"] = struct{}{} } } else { allowed["password"] = struct{}{} }
	for key := range row { if _, ok := allowed[key]; !ok { return fmt.Errorf("%s listener users[%d]: unsupported field %q", proto, index, key) } }
	if !hasString(row["username"]) { return fmt.Errorf("%s listener users[%d] requires username", proto, index) }
	if uuidMode && !hasString(row["uuid"]) { return fmt.Errorf("%s listener users[%d] requires uuid", proto, index) }; if !uuidMode && !hasString(row["password"]) { return fmt.Errorf("%s listener users[%d] requires password", proto, index) }
	return nil
}
func hasCertificatePair(cfg map[string]interface{}) bool { return hasString(cfg["certificate"]) && hasString(cfg["private-key"]) }
