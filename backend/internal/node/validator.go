...
func fields(values ...string) map[string]struct{} {
	...
	for _, value := range values { out[value] = struct{}{} }
	return out
}
...
func validateSchema(proto string, cfg map[string]interface{}) error {
	...
	if !ok { return fmt.Errorf("no listener schema registered for protocol %s", proto) }
	flat := make(map[string]interface{})
	flattenConfig("", cfg, flat)
	for key := range flat {
		top := strings.SplitN(key, ".", 2)[0]
		if _, ok := allowed[top]; !ok { return fmt.Errorf("%s listener: unsupported field %q", proto, key) }
	}
	return nil
}
...
func flattenConfig(prefix string, value interface{}, out map[string]interface{}) {
	if m, ok := value.(map[string]interface{}); ok {
		for key, child := range m {
			...
			if childMap, ok := child.(map[string]interface{}); ok { flattenConfig(next, childMap, out) } else { out[next] = child }
		}
		return
	}
...
func validateProtocolSpecific(proto string, cfg map[string]interface{}) error {
	...
	case "tuic":
		users := hasNonEmpty(cfg["users"]); token := hasNonEmpty(cfg["token"])
...
func validateCertificateMode(proto string, cfg map[string]interface{}) error {
	cert := hasString(cfg["certificate"]); key := hasString(cfg["private-key"]) || hasString(cfg["private_key"])
	if cert != key { return fmt.Errorf("%s listener requires certificate and private-key together", proto) }
	alternatives := 0
	if cert && key { alternatives++ }
	for _, name := range []string{"reality-config", "shadow-tls", "res-tls", "jls-config"} { if hasNonEmpty(cfg[name]) { alternatives++ } }
	if boolValue(cfg["allow-insecure"]) { alternatives++ }
	if alternatives > 1 { return fmt.Errorf("%s listener has mutually exclusive TLS modes configured", proto) }
	if proto == "anytls" || proto == "hysteria2" || proto == "tuic" || proto == "trusttunnel" {
		if hasNonEmpty(cfg["reality-config"]) || hasNonEmpty(cfg["shadow-tls"]) || hasNonEmpty(cfg["res-tls"]) || hasNonEmpty(cfg["jls-config"]) { return fmt.Errorf("%s listener does not support the selected TLS alternative", proto) }
	}
	return nil
}
