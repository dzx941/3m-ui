...
// NewService constructs the user service explicitly.
...
func (s *Service) Create(in CreateInput) (*models.ProxyUser, error) { username:=strings.TrimSpace(in.Username);if username==""{return nil,errors.New("username is required")};password:=in.Password;if password==""{password,_=randomToken(24)};uuid:=in.UUID;if uuid==""{var err error;uuid,err=newUUID();if err!=nil{return nil,err}};expire:=time.Time{};if in.ExpireTime!=nil{expire=in.ExpireTime.UTC()};enabled:=true;if in.Enabled!=nil{enabled=*in.Enabled};encrypted,err:=encryptPassword(password);if err!=nil{return nil,err};u:=&models.ProxyUser{Username:username,PasswordEncrypted:encrypted,UUID:uuid,TrafficLimit:in.TrafficLimit,ExpireTime:expire,Enabled:enabled};if err:=s.db.Create(u).Error;err!=nil{return nil,fmt.Errorf("create proxy user: %w",err)};return u,nil }
...
func (s *Service) ActiveCredentialsByListener()(map[uint][]Credential,error){result:=make(map[uint][]Credential);var listeners []models.Listener;if err:=s.db.Where("enabled = ?",true).Find(&listeners).Error;err!=nil{return nil,err};for _,listener:=range listeners{before:=listener.Config;if err:=credentials.EnsureListenerCredentials(&listener);err!=nil{return nil,fmt.Errorf("prepare listener %q credentials: %w",listener.Name,err)};if listener.Config!=before{if err:=s.db.Model(&models.Listener{}).Where("id = ?",listener.ID).Update("config",listener.Config).Error;err!=nil{return nil,fmt.Errorf("save generated credentials for listener %q: %w",listener.Name,err)}};creds:=credentialsFromListenerConfig(listener.Protocol,listener.Config);if len(creds)>0{result[listener.ID]=creds}}
	var rows []struct{ListenerID uint;ProxyUserID uint};if err:=s.db.Model(&models.ListenerUser{}).Select("listener_id, proxy_user_id").Where("deleted_at IS NULL").Find(&rows).Error;err!=nil{return nil,err};for _,row:=range rows{if len(result[row.ListenerID])>0{continue};u,err:=s.GetByID(row.ProxyUserID);if err!=nil{if errors.Is(err,gorm.ErrRecordNotFound){continue};return nil,err};if !IsCredentialActive(*u){continue};password,err:=decryptPassword(u.PasswordEncrypted);if err!=nil{return nil,fmt.Errorf("decrypt proxy user %d password: %w",u.ID,err)};result[row.ListenerID]=append(result[row.ListenerID],Credential{Username:u.Username,Password:password,UUID:u.UUID})};return result,nil}
func credentialsFromListenerConfig(protocol,raw string)[]Credential{var cfg map[string]interface{};if strings.TrimSpace(raw)==""||json.Unmarshal([]byte(raw),&cfg)!=nil{return nil};users,ok:=cfg["users"];if !ok{return nil};result:=[]Credential{};switch protocol{case "anytls","hysteria2","mieru","tuic":if m,ok:=users.(map[string]interface{});ok{for username,value:=range m{result=append(result,Credential{Username:username,Password:fmt.Sprint(value),UUID:username})}};default:if list,ok:=users.([]interface{});ok{for _,value:=range list{row,ok:=value.(map[string]interface{});if !ok{continue};result=append(result,Credential{Username:fmt.Sprint(row["username"]),Password:fmt.Sprint(row["password"]),UUID:fmt.Sprint(row["uuid"])})}}};return result}
