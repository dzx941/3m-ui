...
func EnsureListenerCredentials(l *models.Listener) error {
	if l == nil { return fmt.Errorf("listener is nil") }
	...
	if requiresUserCredentials(proto) && !hasExportCredentials(proto, cfg) {
		switch proto {
		case "vless", "vmess":
			uuid, err := randomUUID(); if err != nil { return fmt.Errorf("generate client uuid: %w", err) }
			cfg["users"] = []interface{}{map[string]interface{}{"username": "client", "uuid": uuid}}
		case "trojan", "hysteria2", "anytls", "mieru", "shadowquic", "tuic":
			password, err := randomSecret(24); if err != nil { return fmt.Errorf("generate client credential: %w", err) }
			username := "client"
			if proto == "tuic" { username, err = randomUUID(); if err != nil { return fmt.Errorf("generate TUIC client uuid: %w", err) } }
			cfg["users"] = map[string]interface{}{username: password}
		}
		encoded, err := json.Marshal(cfg); if err != nil { return fmt.Errorf("encode listener credentials: %w", err) }
...
func requiresUserCredentials(proto string) bool {
	switch strings.ToLower(proto) { case "vless", "vmess", "trojan", "hysteria2", "anytls", "mieru", "shadowquic", "tuic": return true; default: return false }
}
func hasExportCredentials(proto string, cfg map[string]interface{}) bool {
	users, ok := cfg["users"]; if !ok || users == nil { return false }
	switch strings.ToLower(proto) {
	case "hysteria2", "anytls", "mieru", "tuic": m, ok := users.(map[string]interface{}); return ok && len(m) > 0
	default: list, ok := users.([]interface{}); return ok && len(list) > 0
	}
}
