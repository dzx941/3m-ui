...
func (b *Bot) restoreWizard(chatID string) {
	b.wizardMu.Lock()
	if _, ok := b.wizards[chatID]; ok { b.wizardMu.Unlock(); return }
	b.wizardMu.Unlock()
	state, err := loadWizardState(b.db, chatID)
	if err != nil { return }
	if state != nil {
		b.wizardMu.Lock()
		if _, ok := b.wizards[chatID]; !ok { b.wizards[chatID] = state }
		b.wizardMu.Unlock()
	}
}

func (b *Bot) persistWizard(chatID string) {
	b.wizardMu.Lock()
	state, ok := b.wizards[chatID]
	b.wizardMu.Unlock()
	if ok {
		if err := saveWizardState(b.db, chatID, *state); err != nil { log.Printf("telegram: persist wizard %s: %v", chatID, err) }
	}
}

func (b *Bot) clearWizard(chatID string) {
	b.wizardMu.Lock()
	delete(b.wizards, chatID)
	b.wizardMu.Unlock()
	if err := clearWizardState(b.db, chatID); err != nil { log.Printf("telegram: clear wizard %s: %v", chatID, err) }
}
...
func (b *Bot) loop() {
	...
	for {
		...
		if commandsToken != settings.BotToken {
			if err := setCommandsWithRetry(tgClient, 5); err != nil { log.Printf("telegram: set commands failed after retries: %v", err) } else { commandsToken = settings.BotToken }
		}
		...
		for _, u := range updates {
			...
			next := u.UpdateID + 1
			if next > offset {
				if err := SaveUpdateOffset(b.db, next); err != nil { log.Printf("telegram: persist update offset %d: %v", next, err); break }
				offset = next
			}
		}
	}
...
func (b *Bot) processUpdate(c *Client, settings Settings, u tgUpdate) error {
	...
	if strings.EqualFold(fields[0], "/cancel") {
...
func (b *Bot) handleCallbackGuarded(c *Client, s Settings, chatID, callbackID, data string) error {
	...
	if data == "admin:add" || strings.HasPrefix(data, "add:") {
		b.wizardMu.Lock()
		_, active := b.wizards[chatID]
		b.wizardMu.Unlock()
		if active { return c.AnswerCallback(callbackID, "Wizard already active / 向导已在进行中") }
	}
	if err := b.handleCallback(c, current, chatID, callbackID, data); err != nil { return err }
	b.persistWizard(chatID)
	return nil
}
...
type tgUpdatesResponse struct {
	...
	Parameters *struct { RetryAfter int `json:"retry_after"` } `json:"parameters,omitempty"`
}
...
func getUpdates(httpClient *http.Client, token string, offset int64, timeoutSec int) ([]tgUpdate, int64, error) {
	...
	for attempt := 0; attempt < 4; attempt++ {
		...
		if err != nil {
			if attempt < 3 { time.Sleep(time.Duration(1<<attempt) * time.Second); continue }
			return nil, offset, err
		}
		raw, readErr := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
		resp.Body.Close()
		if readErr != nil { return nil, offset, readErr }
		if resp.StatusCode >= 500 || resp.StatusCode == 429 {
			var p tgUpdatesResponse
			_ = json.Unmarshal(raw, &p)
			delay := time.Duration(1<<attempt) * time.Second
			if p.Parameters != nil && p.Parameters.RetryAfter > 0 { delay = time.Duration(p.Parameters.RetryAfter) * time.Second }
			if attempt < 3 { time.Sleep(delay); continue }
			return nil, offset, fmt.Errorf("telegram API HTTP %d: %s", resp.StatusCode, strings.TrimSpace(string(raw)))
		}
	...
	return nil, offset, fmt.Errorf("telegram getUpdates retries exhausted")
}
