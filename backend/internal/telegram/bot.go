	...
	for {
		...
		for _, u := range updates {
			if u.CallbackQuery != nil {
				...
				if err := b.handleCallback(tgClient, settings, chatID, u.CallbackQuery.ID, u.CallbackQuery.Data); err != nil { log.Printf("telegram: callback: %v", err) }
				continue
			}
...
func getUpdates(httpClient *http.Client, token string, offset int64, timeoutSec int) ([]tgUpdate, int64, error) {
	...
	var parsed struct { OK bool `json:"ok"`; Result []tgUpdate `json:"result"`; Description string `json:"description"` }
	if err := json.Unmarshal(raw, &parsed); err != nil { return nil, offset, err }
	if !parsed.OK { return nil, offset, fmt.Errorf("telegram getUpdates: %s", parsed.Description) }
	next := offset
