...
func (s *Service) Update(totalUpload, totalDownload int64, connections int) Snapshot {
	...
	result := Snapshot{UploadBytes: totalUpload, DownloadBytes: totalDownload, Connections: connections}
	if seconds > 0 {
...
func (s *Service) ApplySample(totalUpload, totalDownload int64, connections int, uploadRate, downloadRate int64) Snapshot {
	s.mu.Lock()
	defer s.mu.Unlock()
	result := Snapshot{UploadBytes: totalUpload, DownloadBytes: totalDownload, Connections: connections, UploadRate: uploadRate, DownloadRate: downloadRate}
	s.last = result
	s.lastTime = time.Now()
	return result
}
