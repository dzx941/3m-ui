...
func main() {
	...
	cfg, err := config.LoadConfig(configPath); if err != nil { log.Fatalf("load config: %v", err) }
	if _, err := database.InitDB(cfg.Database.Path); err != nil { log.Fatalf("initialize database: %v", err) }
	if created, username, password, err := auth.EnsureAdmin(database.GlobalDB, cfg.Database.Path); err != nil { log.Fatalf("initialize administrator: %v", err) } else if created {
	...
	// Wire the credential provider at the composition root. This keeps the
	// config package independent from user/node packages and removes the Go
	// import cycle while preserving legacy-user fallback for old databases.
	dbconfig.CredentialProvider = func() (map[uint][]dbconfig.Credential, error) {
		if user.GlobalService == nil { return map[uint][]dbconfig.Credential{}, nil }
		provided, err := user.GlobalService.ActiveCredentialsByListener(); if err != nil { return nil, err }
	...
	if err := r.Run(addr); err != nil { log.Fatalf("run server: %v", err) }
}
...
func mountFrontend(r *gin.Engine) {
	staticFS, err := fs.Sub(frontendFiles, "web/dist"); if err != nil { log.Printf("frontend assets unavailable: %v", err); return }
	fileServer := http.FileServer(http.FS(staticFS))
	r.RedirectTrailingSlash = false; r.RedirectFixedPath = false
