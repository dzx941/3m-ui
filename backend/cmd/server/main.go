...
func main() {
	...
	if created, username, password, err := auth.EnsureAdmin(database.GlobalDB, cfg.Database.Path); err != nil {
		...
		// Keep the first-install credential available to the operator without
		// printing the password into service logs. This file is removed by the
		// uninstall script together with the configuration directory/data when
		// --purge is used.
