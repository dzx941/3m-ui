...
const ClientAccessPage: React.FC = () => {
  ...
  const showQrCode = (url: string) => {
    setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=250x200&data=${encodeURIComponent(url)}`);
    setQrOpen(true);
  };
  ...
  return (
    <div>
      ...
      >
        <img src={qrUrl} alt="QR Code" style={{ maxWidth: '100%', height: 'auto', display: 'block', margin: '0 auto' }} />
      </Modal>
    </div>
  );
};
