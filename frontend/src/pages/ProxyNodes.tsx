...
export default function ProxyNodes() {
  ...
  const handleGenerateLink = async () => {
    try {
      ...
      const res = await apiRequest<{ token: string; clash_link: string; singbox_link: string; shadowrocket_link: string }>('/access-tokens', {
        method: 'POST',
        body: JSON.stringify(payload),
      });

      // Use server-generated links so PUBLIC_URL and reverse-proxy schemes are respected.
      setGeneratedLinks({
        clash: res.clash_link,
        singbox: res.singbox_link,
        shadowrocket: res.shadowrocket_link,
      });
